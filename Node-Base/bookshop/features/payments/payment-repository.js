...
const insertInternalPayment = async( paymentInfo ) => {
    const {id, user_id, provider_id, method, transaction_id, amount, receipt} = paymentInfo;
    ...
    const query = `
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?)
    `;
    await db.query(query,[id, user_id, provider_id, method, transaction_id, amount, receipt]);
...
const insertPGPayment = async( paymentInfo) => {
    const {paymentBid, userBid, providerBid, paymentMethod, transactionId, amount, receipt} = paymentInfo;
    const query = `
        INSERT INTO payments (id, user_id, provider_id, method, transaction_id, amount, receipt)
        VALUES (?, ?, ?, ?, ?, ?, ?)
    `;
    await db.query(query,[paymentBid, userBid, providerBid, paymentMethod, transactionId, amount, receipt]);
    return id;
}

module.exports = { insertPaymentProvider, insertInternalPayment, insertPGPayment }