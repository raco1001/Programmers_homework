...
const { insertPaymentProvider, insertInternalPayment, insertPGPayment } = require('./payment-repository');
...
const createPaymentProvider = (providerName, providerType) => {
    ...
    const id = generatePK().binaryId;
        console.log(id);
    const insertPaymentProviderResult =  insertPaymentProvider(id, providerName, providerType);
...
const processPayment = async (paymentInfos) => {
    ...
    try {
        ...
        const { userId, providerId, addressId, paymentMethod, paymentType, data } = paymentInfos;
